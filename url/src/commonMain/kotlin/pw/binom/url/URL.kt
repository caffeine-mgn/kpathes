...
value class URL internal constructor(val fullPath: String) {
    ...
    val host: HostPort
        get() {
            val domain: String
            val port: Int?
            UrlHelper.parseUrl(fullPath, domain = { domain = it }, port = { port = it })
            return if (port == null) {
                HostPort(domain)
            } else {
                HostPort(domain = domain, port = port)
            }
        }
    val domain: String
        get() {
    ...
    fun goto(other: URI): URL {
        ...
        if (schema != this.schema && schema.isNotEmpty()) {
            return other.toURL(schema = schema)
        }
        val fullpath = other.path
        return when {
            ...
            other.schema == null -> {
                ...
                other.toURL(schema = schema)
            }
        }
        val host = fullpath.elements[0]
        val hostElement = host.split(':', limit = 2)
        val domain = hostElement[0]
        val port = hostElement.getOrNull(1)?.toInt()
